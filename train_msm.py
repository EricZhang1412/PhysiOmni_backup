...
def evaluate(model, tokenizer, dataloader):
    ...
    for _, (batch) in enumerate(dataloader):
        ...
        with ctx:
            EEG_private_codebook_indices, EOG_private_codebook_indices, ECG_private_codebook_indices, EMG_private_codebook_indices, \
            EEG_shared_codebook_indices, EOG_shared_codebook_indices, ECG_shared_codebook_indices, EMG_shared_codebook_indices = tokenizer.get_codebook_indices(data)

        data.update({
            'EEG_private_codebook_indices': EEG_private_codebook_indices.view(-1) if EEG_private_codebook_indices is not None else None,
            'EOG_private_codebook_indices': EOG_private_codebook_indices.view(-1) if EOG_private_codebook_indices is not None else None,
            'ECG_private_codebook_indices': ECG_private_codebook_indices.view(-1) if ECG_private_codebook_indices is not None else None,
            'EMG_private_codebook_indices': EMG_private_codebook_indices.view(-1) if EMG_private_codebook_indices is not None else None,
            'EEG_shared_codebook_indices': EEG_shared_codebook_indices.view(-1) if EEG_shared_codebook_indices is not None else None,
            'EOG_shared_codebook_indices': EOG_shared_codebook_indices.view(-1) if EOG_shared_codebook_indices is not None else None,
            'ECG_shared_codebook_indices': ECG_shared_codebook_indices.view(-1) if ECG_shared_codebook_indices is not None else None,
            'EMG_shared_codebook_indices': EMG_shared_codebook_indices.view(-1) if EMG_shared_codebook_indices is not None else None,
        })
...
def main(args):
    ...
    checkpoint_out_dir = os.path.join(args.out_dir, 'checkpoints/MSM')
    if master_process:
    ...
    # load tokenizer from a checkpoint
    tokenizer_ckpt_path = os.path.join(args.out_dir, args.tokenizer_dir, 'ckpt-99.pt')
    tokenizer_checkpoint = torch.load(tokenizer_ckpt_path, map_location=device)
    tokenizer_EEG_checkpoint_model_args = tokenizer_checkpoint['EEG_encoder_args']
    tokenizer_EOG_checkpoint_model_args = tokenizer_checkpoint['EOG_encoder_args']
    tokenizer_ECG_checkpoint_model_args = tokenizer_checkpoint['ECG_encoder_args']
    tokenizer_EMG_checkpoint_model_args = tokenizer_checkpoint['EMG_encoder_args']
    # force these config attributes to be equal otherwise we can't even resume training
    # the rest of the attributes (e.g. dropout) can stay as desired from command line
    for k in ['n_layer', 'n_head', 'n_embd', 'block_size', 'bias']:
        EEG_encoder_args[k] = tokenizer_EEG_checkpoint_model_args[k]
        EOG_encoder_args[k] = tokenizer_EOG_checkpoint_model_args[k]
        ECG_encoder_args[k] = tokenizer_ECG_checkpoint_model_args[k]
        EMG_encoder_args[k] = tokenizer_EMG_checkpoint_model_args[k]
    tokenizer_EEG_checkpoint_model_args = tokenizer_checkpoint['EEG_decoder_args']
    tokenizer_EOG_checkpoint_model_args = tokenizer_checkpoint['EOG_decoder_args']
    tokenizer_ECG_checkpoint_model_args = tokenizer_checkpoint['ECG_decoder_args']
    tokenizer_EMG_checkpoint_model_args = tokenizer_checkpoint['EMG_decoder_args']
    for k in ['n_layer', 'n_head', 'n_embd', 'block_size', 'bias']:
        EEG_decoder_args[k] = tokenizer_EEG_checkpoint_model_args[k]
        EOG_decoder_args[k] = tokenizer_EOG_checkpoint_model_args[k]
        ECG_decoder_args[k] = tokenizer_ECG_checkpoint_model_args[k]
        EMG_decoder_args[k] = tokenizer_EMG_checkpoint_model_args[k]
    # create the model
    ...
    tokenizer = VQ(EEG_encoder_conf, EOG_encoder_conf, ECG_encoder_conf, EMG_encoder_conf,
                   EEG_decoder_conf, EOG_decoder_conf, ECG_decoder_conf, EMG_decoder_conf)
    state_dict = tokenizer_checkpoint['model']
    ...
    if init_from == 'scratch':
        ...
        ECG_encoder_conf = NTConfig(**ECG_encoder_args)
        EMG_encoder_conf = NTConfig(**EMG_encoder_args)
        model = MSM(EEG_encoder_conf, EOG_encoder_conf, ECG_encoder_conf, EMG_encoder_conf)
        start_epoch = 0
        best_val_loss = 999
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # validation
        log_val = evaluate(model, tokenizer, data_loader_val)
...
def get_args():
    ...
    parser.add_argument('--wandb_runname', default='MSM')
